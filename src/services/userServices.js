    ...
    try {
      ...
      if (isExist) {
        ...
        if (user) {
          ...
          if (check) {
            userData.errCode = 0;
            (userData.errMessage = "Login successful"), delete user.password;
            userData.user = user;
            token = jwt.sign(
        ...
        userData.errCode = 1;
        userData.errMessage = `Yours's email not exist`;
      }
      resolve(
        userData
        );
    ...
    try {
      ...
      if (users.image && users) {
        for (let i = 0; i < users.length; i++) {
          users[i].image = new Buffer(users[i].image, "base64").toString(
            "binary"
          );
        }
      }
...
let deleteUser = (userId) => {
  return new Promise(async (resolve, reject) => {
    ...
    if (!user) {
      resolve({
        errCode: 2,
        errMessage: "User isn’t existing!",
      });
    }
